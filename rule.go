...
func (r *Rule) Evaluate(tx *Transaction) []MatchData {
	...
	tx.Waf.Logger.WithFields(Fields{
		...
		"txid":     tx.ID,
		"event":    "EVALUATE_RULE",
		"is_chain": r.ParentID != 0,
	}).Debug("Evaluating rule")
	defer tx.Waf.Logger.WithFields(Fields{
		"rule":           rid,
		"matched_values": len(matchedValues),
		"txid":           tx.ID,
		"event":          "FINISH_RULE",
		"is_chain":       r.ParentID != 0,
	}).Debug("finished evaluating rule")
			...
			for _, arg := range values {
				...
				if len(errs) > 0 {
					tx.Waf.Logger.WithFields(Fields{
						...
						"argument": arg.Value,
						"errors":   errs,
					}).Error("Error transforming argument")
				}
				tx.Waf.Logger.WithFields(Fields{
					"rule":     rid,
					"txid":     tx.ID,
					"argument": arg.Value,
				}).Debug("Arguments transformed")
				...
				for _, carg := range args {
					...
					tx.Waf.Logger.WithFields(Fields{
						...
						"operator": r.operator.Function, // TODO fix
						"data":     carg,
						"variable": arg.Variable.Name(),
						"key":      arg.Key,
						"value":    carg,
						"result":   match,
					}).Debug("Evaluate rule operator")
				}
			}
		}
	}
