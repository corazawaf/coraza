		...
		if tx.SkipAfter != "" {
			if r.SecMark == tx.SkipAfter {
				tx.Waf.Logger.WithFields(Fields{
					"txid":    tx.ID,
					"secmark": r.SecMark,
					"event":   "FINISH_SECMARK",
				}).Debug("SkipAfter was finished")
				tx.SkipAfter = ""
			} else {
				tx.Waf.Logger.WithFields(Fields{
					"txid":    tx.ID,
					"rule":    r.ID,
					"secmark": tx.SkipAfter,
					"event":   "SKIP_RULE_BY_SECMARK",
				}).Debug("Skipping rule because of SkipAfter")
			}
			continue
		}
		if tx.Skip > 0 {
