...
func (a *prependFn) Evaluate(r rules.RuleMetadata, txS rules.TransactionState) {
	...
	if err != nil {
		tx.WAF.Logger.Debug("failed to write buffer while evaluating prepend action: %w", err)
	}
	reader, err := tx.ResponseBodyBuffer.Reader()
	if err != nil {
		tx.WAF.Logger.Debug("failed to read response body while evaluating prepend action: %w", err)
	}
	_, err = io.Copy(buf, reader)
	if err != nil {
		tx.WAF.Logger.Debug("failed to append response buffer while evaluating prepend action: %w", err)
	}
