...
type Transaction struct {
	...
	// Contains duration in microseconds per phase
...
// bodyLimitReached handles the case where the body buffer has already reached its limit.
// Returns (interruption, shouldReturn). When shouldReturn is true, the caller should
// return the interruption (which may be nil for ProcessPartial).
func (tx *Transaction) bodyLimitReached(bufLen, limit int64, limitAction types.BodyLimitAction) (*types.Interruption, bool) {
	if limit == bufLen {
		if limitAction == types.BodyLimitActionReject {
			return tx.interruption, true
		}
		if limitAction == types.BodyLimitActionProcessPartial {
			return nil, true
		}
	}
	return nil, false
...
// checkBodyOverflow checks whether writing writingBytes to a body buffer of bufLen would exceed limit.
// It performs an integer overflow check and returns:
//   - clampedBytes: the number of bytes safe to write
//   - runProcess: whether the body processor should run (limit reached with ProcessPartial)
//   - interruption: non-nil if the transaction should be interrupted
//   - err: non-nil on integer overflow
func (tx *Transaction) checkBodyOverflow(
	bufLen, writingBytes, limit int64,
	limitAction types.BodyLimitAction,
	rejectStatus int,
	dataErrorVar *collections.Single,
) (clampedBytes int64, runProcess bool, interruption *types.Interruption, err error) {
	// Overflow check: MaxInt64 is not a realistic payload size
	if bufLen >= (math.MaxInt64 - writingBytes) {
		return 0, false, nil, errors.New("overflow reached while writing body")
	}

	if bufLen+writingBytes >= limit {
		dataErrorVar.Set("1")
		if limitAction == types.BodyLimitActionReject {
			interruption, _, _ := setAndReturnBodyLimitInterruption(tx, rejectStatus)
			return 0, false, interruption, nil
		}
		if limitAction == types.BodyLimitActionProcessPartial {
			return limit - bufLen, true, nil, nil
		}
	}
	return writingBytes, false, nil, nil
}
...
func (tx *Transaction) WriteRequestBody(b []byte) (*types.Interruption, int, error) {
	...
	if it, shouldReturn := tx.bodyLimitReached(tx.requestBodyBuffer.length, tx.RequestBodyLimit, tx.WAF.RequestBodyLimitAction); shouldReturn {
		return it, 0, nil
	}

	writingBytes, runProcess, it, err := tx.checkBodyOverflow(
		tx.requestBodyBuffer.length, int64(len(b)), tx.RequestBodyLimit,
		tx.WAF.RequestBodyLimitAction, 413, tx.variables.inboundDataError,
	)
	if err != nil {
		return nil, 0, err
	}
...
func (tx *Transaction) ReadRequestBodyFrom(r io.Reader) (*types.Interruption, int, error) {
	...
	if l, ok := r.(ByteLenger); ok {
		...
		var err error
		writingBytes, runProcess, it, err = tx.checkBodyOverflow(
			tx.requestBodyBuffer.length, int64(l.Len()), tx.RequestBodyLimit,
			tx.WAF.RequestBodyLimitAction, 413, tx.variables.inboundDataError,
		)
		if err != nil {
			return nil, 0, err
...
func (tx *Transaction) ProcessRequestBody() (*types.Interruption, error) {
	...
	if err := tx.selectAndRunRequestBodyProcessor(reader); err != nil {
		return nil, err
	}
...
// selectAndRunRequestBodyProcessor resolves the body processor name, looks up the processor,
// and runs it against the given reader. It returns an error only for unexpected failures.
func (tx *Transaction) selectAndRunRequestBodyProcessor(reader io.Reader) error {
	mime := ""
	if m := tx.variables.requestHeaders.Get("content-type"); len(m) > 0 {
		mime = m[0]
	}

	bodyProcessorName := tx.variables.reqbodyProcessor.Get()
	...
	if tx.ForceRequestBodyVariable {
		// We force URLENCODED if mime is x-www... or we have an empty body processor and ForceRequestBodyVariable
		if bodyProcessorName == "" {
			bodyProcessorName = "URLENCODED"
		}
		tx.variables.reqbodyProcessor.Set(bodyProcessorName)
	}
	bodyProcessorName = strings.ToLower(bodyProcessorName)
	if bodyProcessorName == "" {
		return nil
	}
	bp, err := bodyprocessors.GetBodyProcessor(bodyProcessorName)
	if err != nil {
	...
	tx.debugLogger.Debug().
		Str("body_processor", bodyProcessorName).
		Msg("Attempting to process request body")

	if err := bp.ProcessRequest(reader, tx.Variables(), plugintypes.BodyProcessorOptions{
		Mime:        mime,
...
func (tx *Transaction) ProcessResponseHeaders(code int, proto string) *types.Interruption {
	...
	tx.variables.responseStatus.Set(statusCode)
...
func (tx *Transaction) WriteResponseBody(b []byte) (*types.Interruption, int, error) {
	...
	if it, shouldReturn := tx.bodyLimitReached(tx.responseBodyBuffer.length, tx.ResponseBodyLimit, tx.WAF.ResponseBodyLimitAction); shouldReturn {
		return it, 0, nil
	}

	writingBytes, runProcess, it, err := tx.checkBodyOverflow(
		tx.responseBodyBuffer.length, int64(len(b)), tx.ResponseBodyLimit,
		tx.WAF.ResponseBodyLimitAction, 500, tx.variables.outboundDataError,
	)
	if err != nil {
		return nil, 0, err
	}
...
func (tx *Transaction) ReadResponseBodyFrom(r io.Reader) (*types.Interruption, int, error) {
	...
	if l, ok := r.(ByteLenger); ok {
		...
		var err error
		writingBytes, runProcess, it, err = tx.checkBodyOverflow(
			tx.responseBodyBuffer.length, int64(l.Len()), tx.ResponseBodyLimit,
			tx.WAF.ResponseBodyLimitAction, 500, tx.variables.outboundDataError,
		)
		if err != nil {
			return nil, 0, err
...
func (tx *Transaction) ProcessResponseBody() (*types.Interruption, error) {
	...
	if err := tx.selectAndRunResponseBodyProcessor(reader); err != nil {
		return tx.interruption, err
	}
...
// selectAndRunResponseBodyProcessor resolves the response body processor and runs it,
// or falls back to reading the response body into variables.
func (tx *Transaction) selectAndRunResponseBodyProcessor(reader io.Reader) error {
...
func (tx *Transaction) AuditLog() *auditlog.Log {
	...
	clientPort, err := strconv.Atoi(tx.variables.remotePort.Get())
	if err != nil {
		tx.debugLogger.Debug().Err(err).Msg("Error converting client port to integer")
	}
	hostPort, err := strconv.Atoi(tx.variables.serverPort.Get())
	if err != nil {
		tx.debugLogger.Debug().Err(err).Msg("Error converting host port to integer")
	}
	...
	if !rulesMatchedSet && includeErrorLog {
		for _, matchedRule := range tx.matchedRules {
			loggableRule, ok := matchedRule.(*corazarules.MatchedRule)
			if ok && loggableRule.Log() {
				al.Messages_ = append(al.Messages_, auditlog.Message{
					ErrorMessage_: matchedRule.ErrorLog(),
				})
			}
		}
	}
...
func (tx *Transaction) auditLogPartRequestHeaders(al *auditlog.Log) {
	al.Transaction_.Request_.Headers_ = tx.variables.requestHeaders.Data()
}

func (tx *Transaction) auditLogPartRequestBody(al *auditlog.Log) {
	reader, err := tx.requestBodyBuffer.Reader()
	if err == nil {
...
func (tx *Transaction) auditLogPartResponseBody(al *auditlog.Log) {
...
func (tx *Transaction) auditLogPartResponseHeaders(al *auditlog.Log) {
	if al.Transaction_.Response_ == nil {
		al.Transaction_.Response_ = &auditlog.TransactionResponse{}
	}
	responseStatus, err := strconv.Atoi(tx.variables.responseStatus.Get())
	if err != nil {
		tx.debugLogger.Debug().Err(err).Msg("Error converting response status to integer")
	}
	al.Transaction_.Response_.Status_ = responseStatus
	al.Transaction_.Response_.Headers_ = tx.variables.responseHeaders.Data()
}

func (tx *Transaction) auditLogPartProducer(al *auditlog.Log) {
...
func (tx *Transaction) auditLogPartMatchedRules(al *auditlog.Log, includeErrorLog bool) {
	for _, matchedRule := range tx.matchedRules {
		...
		if ok && loggableRule.Log() {
			r := matchedRule.Rule()
			for _, matchData := range matchedRule.MatchedDatas() {
				newAlEntry := auditlog.Message{
					...
					Data_: &auditlog.MessageData{
						File_:     matchedRule.Rule().File(),
						Line_:     matchedRule.Rule().Line(),
						ID_:       r.ID(),
						Rev_:      r.Revision(),
				...
				// in the audit log
				if includeErrorLog {
					newAlEntry.ErrorMessage_ = matchedRule.ErrorLog()
				}
