...
// each CRS attack category, using official FTW attack payloads mixed with
// benign traffic (50/50 split). Run from the repo root:
//
...
func BenchmarkCRSPrefilterVsRegex(b *testing.B) {
	...
	// Group rules by category. For each category collect all payloads
	// (attack + benign interleaved).
	...
	for _, rule := range rules {
		...
		// Mix attack payloads with benign payloads.
		var payloads []string
