...
func TestCaptureGroups(t *testing.T) {
	...
	if tx.GetCollection("tx").GetData()["0"][0] != str {
		t.Errorf("Invalid capture 0")
	}
	if tx.GetCollection("tx").GetData()["1"][0] != "www.google.com" {
		t.Errorf("Invalid capture 1, got " + tx.GetCollection("tx").GetData()["1"][0])
	}
	...
	if tx.GetCollection("tx").GetData()["1"][0] != "4" {
		t.Errorf("Invalid capture @rx for range")
	}
	if tx.GetCollection("tx").GetData()["2"][0] != "5" {
		t.Errorf("Invalid capture @rx for range")
	}
}
